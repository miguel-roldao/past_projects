...
double Spline3Interpolator::Deriv1Interpolate(double fx) {
  ...
  C = K[i]/6 * (3 * pow(fx-x[i+1],2)/(x[i]-x[i+1]) - (x[i] - x[i+1])) - K[i+1]/6 * (3 * pow(fx-x[i],2)/ (x[i]-x[i+1]) - (x[i] - x[i+1]))  + (y[i] - y[i+1])/(x[i] - x[i+1]);

  return C;
}

TF1* Spline3Interpolator::GetDeriv1InterpolFunction()
{
  Deriv1 = new TF1("1st Derivative", this, &Spline3Interpolator::f_linhaInterpolator, x[0]-0.001 ,x[N-1]+0.001, 0);
