...
gchar               chlabel[48] = "Valor actual (Limites: [-5,7]): " ;
         ...
         gpointer   label  )
{
  ...
  if (contador >= contaLim[1])
    gtk_widget_set_sensitive (bplus, FALSE);

  if (!gtk_widget_get_sensitive (bminus))
    gtk_widget_set_sensitive (bminus, TRUE);
          ...
          gpointer   label  )
{
  ...
  if (contador <= contaLim[0])
    gtk_widget_set_sensitive (bminus, FALSE);

  if (!gtk_widget_get_sensitive (bplus))
    gtk_widget_set_sensitive (bplus, TRUE);
