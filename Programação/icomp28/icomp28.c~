...
typedef struct
{
  gboolean      start; 
  gboolean      pausa;  
  double        time;       // guarda o valor do tempo quando a função está em 'pause'
...
{
  GtkWidget *garea;
  double    *x[2], *y[2];    // vetores onde são guardados os valores dos gráficos 
  double    scale[2];        // escala o valor dos gráficos para os podermos ver na drawing area
  double    rgb[2][3];       // cores dos gráficos
  gdouble   ZOOM_X, ZOOM_Y;  // permite alterar as escalas
  int       n;               // dimensão do vetor onde são guardados os valores
  char      id[2];           // identifica a escolha da 'combobox' dos gráficos
  char      active_graph[2]; // identifica os gráficos que estão a ser desenhados
  gboolean  show_value;      // controla se o valor é ou não impresso no canto sup. esq. do ecrã
} graph;
...
{ 
  ...
  double          dphi0; // velocidade inicial em phi
  double          theta;
  double          theta0;
  ...
  double          color[3][3];    // cores das massas
  char            active_radio;   // masssa escolhida (para alterar a cor)
  gboolean        move       ;    // permite so haver movimento com o timer
  graph           *graph;         // ponteiro para uma estrutura graph
  status          *stat;          // ponteiro para a estrutura da status
  GtkWidget       *darea     ;    // tem fazer parte da struct porque g_timeout_add só recebe um argumento
  GtkWidget       *window;        
  gdouble          win_xlen, win_ylen; // dimensões da janela
} data ;

typedef struct
{
  GtkWidget  *scale;     // widget GTK_RANGE
  GtkWidget *label;
  gdouble ScaleInitValue; 
  char *ch_LabelValue;
  gdouble min;
  gdouble max;
  gdouble step;
...
gboolean on_draw_event (GtkWidget *drawing_area, cairo_t *cr, data *pdata)
{
  ...
  if (pdata->graph->show_value)
      ...
      if (pdata->graph->active_graph[0] != 10)
	{
	  ...
	  sprintf(text4, " t :   %3.2lf s", pdata->t); // O Tempo é da Simulação! ~ 2 x mais rápido que o real
	  ...
	  if (dummy == 1)
	    ...
	    {
	      cairo_move_to(cr, 30, 38);
	      cairo_show_text(cr, text4); // para mostrar o valor 'real'
	    }
	}
  ...
  // Mostra o esquema explicativo (em 'pause')
...
gboolean time_handler (data *data)
{
  ...
  gtk_widget_queue_draw (data->darea); // para permitir mudar os valores das massas e comprimentos em 'pause'
...
GtkWidget *cria_scale (Scale *s)
{
  ...
  gtk_range_set_value (GTK_RANGE(w), s->ScaleInitValue);
...
gboolean cb_change_scale (GtkWidget *w, Scale *s)  
{
  ...
  switch (s->id)
    ...
    case 9:
      ...
      sprintf(str1, s->ch_LabelValue, s->pdata->phi0 * 57.3); // converte de rad para º
      break;
...
gboolean draw_graph (GtkWidget *garea, cairo_t *cr, data *gdata)
{
  ...
  i++;                 // funciona como um 'for' gigante (cada vez que a função é chamada)
...
int main (int argc, char **argv)
{
...
#if GTK_CHECK_VERSION(3,22,0)
  gchar *cssData = "radiobutton:checked {background-color: Snow;}";
#else
  gchar *cssData = "GtkRadioButton:checked {background-color: Snow;}";
#endif
  ...
  // Criação e definição do ícone

  icon = gdk_pixbuf_new_from_file("icon.png", NULL);
  gtk_window_set_icon(GTK_WINDOW(pdata->window), icon);
  g_object_unref(icon);
  ...
  //  Botão de 'Pause'/'Play' : semelhante ao funcionamento do Restart Button (ver abaixo)
 ...
 // Criação da 'label' para o Gráfico 1
  ...
  // Criação e configuração dos 'radio button'
...
/****                The Swing
	 ...
	 Up in the air I go flying again,
	   ...
	   - Robert Louis Stevenson, A Child's Garden of Verses (1999) ****/
